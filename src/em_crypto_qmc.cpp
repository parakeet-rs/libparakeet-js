...
uint16_t create_qmc_v2_ekey_raw(EM_ARG_VEC(ekey))
{
    if (len_ekey > 300)
    {
        return g_transformer_registry.add(transformer::CreateQMC2RC4DecryptionTransformer(EM_VEC_PTR_LEN(ekey)));
    }
    else
    {
        return g_transformer_registry.add(transformer::CreateQMC2MapDecryptionTransformer(EM_VEC_PTR_LEN(ekey)));
    }
}
...
uint16_t create_qmc_v2_ekey(EM_ARG_VEC(ekey), uint16_t key_crypto_handle)
{
    ...
    {
        auto ekey_raw = key_crypto->Decrypt(EM_VEC_PTR_LEN(ekey));
        return create_qmc_v2_ekey_raw(reinterpret_cast<uintptr_t>(ekey_raw.data()), ekey_raw.size());
    }
...
EMSCRIPTEN_BINDINGS(EM__CryptoQMC)
{
    ...
    function("create_qmc_v2_ekey_raw", &create_qmc_v2_ekey_raw);
}
