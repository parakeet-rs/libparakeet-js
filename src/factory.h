...
public:
  inline uint16_t add(std::shared_ptr<T> transformer) {
    ...
    do {
      next_id_ += delta;
             ...
             transformers_.find(next_id_) != transformers_.end());
  ...
  inline void close(uint16_t id) { transformers_.erase(id); }
...
extern TransformerRegistry<parakeet_crypto::ITransformer>
    g_transformer_registry;

extern TransformerRegistry<parakeet_crypto::qmc2::QMCFooterParser>
    g_qmc2_footer_parser;
