...
EMSCRIPTEN_BINDINGS(EM__RW_Interfaces)
{
    class_<IReadSeekable>("IReadSeekable")
        .function("Read", select_overload<size_t(uint8_t *, size_t)>(&IReadSeekable::Read), pure_virtual(),
                  allow_raw_pointers())
        .function("Seek", &IReadSeekable::Seek, pure_virtual(), allow_raw_pointers())
    ...
    class_<IWriteable>("IWriteable")
        .function("Write", &IWriteable::Write, pure_virtual(), allow_raw_pointers())
